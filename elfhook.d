...
@nogc:  // since this code is used by perl.
...
int readSectionTable(int fd, const Elf_Ehdr* header, ref Elf_Shdr* table)
{
  ...
  if (read(fd, table, size) <= 0) {  // table size should > 0.
    free(table);
    return errno = EINVAL;
  }
...
int readStringTable(int fd, const Elf_Shdr* section, ref char* strings)
{
  ...
  if (read(fd, cast(char*) strings, section.sh_size) <= 0) { // ditto.
    free(cast(void*) strings);
    return errno = EINVAL;
  }
...
void* hook(const char* filename, const char* functionName, const void* substitutionAddress)
{
  ...
  auto ret = elfHook(filename, address, functionName, substitutionAddress);
  return ret;
}
